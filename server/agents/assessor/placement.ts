...
export async function createPlacement(store: WorkspaceRepository, learnerId: string, goalId: string) {
  ...
  if (existing && existing.questions.length >= 10) return publicAssessment(existing);
  ...
  if (!generated) throw new Error(`The placement check could not be completed: ${lastError instanceof Error ? lastError.message : 'unknown error'}`);
