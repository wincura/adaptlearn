  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      try {
        ...
        if (error) setToast(error === 'import' ? 'Signed in. Your guest progress still needs to be imported. Please retry below.' : error === 'canceled' ? 'Sign-in canceled. Your guest work is still here.' : error === 'not-configured' ? 'Sign-in is not configured yet. Set COGNITO_DOMAIN, COGNITO_CLIENT_ID, and COGNITO_USER_POOL_ID in .env.' : 'Sign-in could not be completed. Please try again.');
