...
function Sidebar({ active, workspace, onNavigate, onManageProfiles, session, onSignIn, onSignOut }: SidebarProps) {
  return <aside className="reference-sidebar">
    ...
    <div className="reference-account"><button className="reference-user" onClick={onManageProfiles} aria-label={session?.kind === 'account' ? 'Edit your profile' : 'Guest learning preferences'}><span>{session?.kind === 'account' && !session.needsName ? initials(workspace.profile.displayName) : <PersonOutlineRounded />}</span><div><strong>{session?.kind === 'account' ? session.needsName ? 'Add your name' : workspace.profile.displayName : 'Guest'}</strong><small>{session?.kind === 'account' ? 'Personal account' : 'Saved in this browser'}</small></div></button>{session?.kind === 'account' ? <button className="reference-auth-button" onClick={onSignOut}>Sign out</button> : <button className="reference-auth-button" disabled={!session?.signInAvailable} title={session?.signInAvailable ? 'Save progress to your account' : 'Sign-in is not configured yet'} onClick={onSignIn}>Sign up / Sign in</button>}</div>
  </aside>;
}

export function ReferenceShell({ active, workspace, children, onNavigate, onManageProfiles, session, onSignIn, onSignOut }: SidebarProps & { children: ReactNode }) {
  return <div className="reference-shell"><Sidebar session={session} onSignIn={onSignIn} onSignOut={onSignOut} active={active} workspace={workspace} onNavigate={onNavigate} onManageProfiles={onManageProfiles} /><main className="reference-main">{children}</main></div>;
}
