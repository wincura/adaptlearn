...
import { isCodingGoal } from '../../shared/topic-detector';
  ...
  return (
    <div className="workspace-canvas">
      ...
      <section className="materials-section">
        ...
        {materials.length === 0 ? <div className="empty-materials"><span><AutoStoriesRounded /></span><h3>Create something you can learn from.</h3><p>{completedPlacement ? 'Choose the focus for your first lesson, or let AdaptLearn decide.' : 'Start with the placement test. Its result determines the difficulty and teaching style of every lesson for this goal.'}</p>{!goal && <button onClick={onAddGoal}>Add your first goal <ArrowForwardRounded /></button>}{goal && <div><button disabled={Boolean(activeAgent)} onClick={onCreateLesson}>{completedPlacement ? <AddRounded /> : <TrackChangesRounded />} {completedPlacement ? 'Create first lesson' : 'Take placement test'}</button>{isCodingGoal(goal) && <button disabled={Boolean(activeAgent)} onClick={() => onAgentAction('builder')}><BoltRounded /> Create practice activity</button>}</div>}</div> : <div className="material-grid">{materials.map((material) => <motion.button initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} onClick={() => onOpenMaterial(material)} className={`material-card ${material.owner}`} key={material.id}><span>{material.kind === 'practice-lab' ? 'PRACTICE ACTIVITY' : 'SOURCED LESSON'}</span><strong>{material.title}</strong><p>{material.summary}</p><small>{material.assessedLevel ? `${material.assessedLevel} · ` : ''}{material.sections.length} sections{material.sources?.length ? ` · ${material.sources.length} sources` : ''} <ArrowForwardRounded /></small></motion.button>)}</div>}
      </section>

      <section className="research-section">
        <div className="section-heading"><div><span>UPDATES &amp; REFRESHERS</span><h2>Useful next, until you approve</h2></div>{goal && <button disabled={Boolean(activeAgent)} onClick={() => onAgentAction('researcher')}><SearchRounded /> What&apos;s new?</button>}</div>
        {!goal ? <p className="quiet-state">Suggestions begin after you add a goal, so they stay relevant.</p> : suggestions.length === 0 ? <p className="quiet-state">{activeAgent === 'researcher' ? 'Checking current sources and your learning needs…' : 'No pending suggestions. Use “What’s new?” for a useful update or refresher.'}</p> : <div className="suggestion-list">{suggestions.map((suggestion) => <article key={suggestion.id}><div><span>{suggestion.purpose === 'update' ? 'NEW OR CHANGED' : suggestion.purpose === 'refresh' ? 'TARGETED REFRESH' : 'NEXT TOPIC'} · OPTIONAL</span><h3>{suggestion.title}</h3><p>{suggestion.summary}</p><small>{suggestion.whyRelevant}</small>{suggestion.sourceUrl && <a href={suggestion.sourceUrl} target="_blank" rel="noreferrer">Open source ↗</a>}</div><button disabled={Boolean(activeAgent)} onClick={() => onAcceptSuggestion(suggestion)}><CheckRounded /> Add to learning path</button></article>)}</div>}
      </section>
    </div>
  );
}
